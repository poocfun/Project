<link href="{{ asset('template') }}/assets/img/favicon.png" rel="icon">
<link href="{{ asset('template') }}/assets/img/apple-touch-icon.png" rel="apple-touch-icon">
{{-- <link rel="stylesheet" href="{{asset('admin')}}/plugins/fontawesome-free/css/all.min.css"> --}}
<script src="https://kit.fontawesome.com/0278088b8e.js" crossorigin="anonymous"></script>
<!-- Google Fonts -->
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link
    href="https://fonts.googleapis.com/css2?family=Open+Sans:ital,wght@0,300;0,400;0,500;0,600;0,700;1,300;1,400;1,600;1,700&family=Amatic+SC:ital,wght@0,300;0,400;0,500;0,600;0,700;1,300;1,400;1,500;1,600;1,700&family=Inter:ital,wght@0,300;0,400;0,500;0,600;0,700;1,300;1,400;1,500;1,600;1,700&display=swap"
    rel="stylesheet">

<!-- Vendor CSS Files -->
<link rel="stylesheet" href="{{ asset('admin') }}/dist/css/adminlte.min.css">
<link href="{{ asset('template') }}/assets/vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">
<link href="{{ asset('template') }}/assets/vendor/bootstrap-icons/bootstrap-icons.css" rel="stylesheet">
<link href="{{ asset('template') }}/assets/vendor/aos/aos.css" rel="stylesheet">
<link href="{{ asset('template') }}/assets/vendor/glightbox/css/glightbox.min.css" rel="stylesheet">
<link href="{{ asset('template') }}/assets/vendor/swiper/swiper-bundle.min.css" rel="stylesheet">
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>


<section class="scroll">
    <script>
        @if (session('error'))
            Swal.fire({
                icon: "error",
                title: "Error",
                text: "{{ session('error') }}",
                confirmButtonText: "OK"
            });
        @endif
    </script>
    <div class="row">
        <div class="col-sm-6 text-black"
            style="background: rgb(5,175,212);
        background: linear-gradient(90deg, rgba(5,175,212,1) 0%, rgba(162,163,181,1) 100%); height: 100%;">
            <div class="d-flex align-items-center h-custom-2 px-5 ms-xl-4 mt-5 pt-5 pt-xl-0 mt-xl-n5">


                <form method="post" action="{{ route('login-user') }}" style="width: 23rem;">
                    @csrf
                    <h3 class="fw-normal" style="letter-spacing: 1px;">Welcome Back</h3>
                    <p>Welcome back, Please enter your details.</p><br>

                    <div class="form-outline mb-4">
                        <label class="form-label" for="form2Example18">Email</label>
                        <input name="email" type="email" id="form2Example18" class="form-control form-control-lg" />
                    </div>

                    <div class="form-outline mb-4">
                        <label class="form-label" for="form2Example28">Password</label>
                        <input name="password" type="password" id="form2Example28"
                            class="form-control form-control-lg" />
                    </div>

                    <div class="form-check d-flex justify-content-between">
                        <input type="checkbox" id="terms" role="button" class="form-check-input d-flex">
                        <label for="terms" class="form-check-label text-white" role="button">Remember for 30
                            Days</label>
                        <div class="d-flex ">
                            <p class="small mb-5 pb-lg-2"><a href="#!"
                                    style="font-weight:bold; color:#00499E; text-decoration: none;">Forgot password?</a>
                            </p>
                        </div>
                    </div>

                    <div class="pt-1 mb-4">
                        <button type="submit" class="btn btn-primary btn-lg btn-block" type="button">Sign in </button>
                    </div>

                    <div class="input-group border rounded fw-bold pt-1 mb-4 justify-content-center bg-white"
                        style="height: 45px;">
                        <img src="data:image/png;base64,"
                            height="35px" alt="">
                        <a href="{{ route('google.login') }}" class="text-dark mt-1"
                            style="text-decoration: none;">Sign In with google</a>
                    </div>

                    <center>
                        <p style="color: white; font-weight:bold;">Don't have an account?
                            <a href="{{ url('registrasi') }}" style="color: #00499E;font-weight:bold;">Sign up
                            </a>
                        </p>

                    </center>

                </form>
            </div>

        </div>
        <div style="background-image: url('{{ asset('template/assets/img/login.jpeg') }}'); background-size: cover;"
            class="col-sm-6 px-0 d-none d-sm-block">
            <center style="margin-top:308px;">
                <h3 style="color: white; font-weight:bold;">Back To Website
                    <a href="{{ url('homee') }}" style="color: #00499E;font-weight:bold;">Click
                    </a>
                </h3>
            </center>
        </div>
    </div>
</section>
